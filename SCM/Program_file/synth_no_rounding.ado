**I noticed that the original synth program is rounding the weights, which creats a substantial difference between pre-treatment synthetic X and treated X
**So I have commented out the two lines that apply the rounding. To see this search for "round"
**there are no other changes made to the program
**I HAVE NOW CREATED ANOTHER VERSION OF synth.ado file called synth_no_rounding.ado THAT AVOIDS ROUNDING WEIGHTS
...
/* round */
**2 lines commented out to avoid rounding
 **mata: roundmat("`wsol'")
 **mat `wsol' = matout
